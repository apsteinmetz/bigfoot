...
```
Load BFRO sighting report from Kaggle dataset. This is sourced from BFRO.net.  I assume the file is parsed from the Google Earth placemarks file found [here](http://www.bfro.net/app/AllReports.kmz).  You will need a Kaggle account to download the files.
...
bfro_data
```
Alternative direct way to get BFRO data.

```{r}
#reports<-  st_read("reports.kml")

#download.file("http://www.bfro.net/app/AllReportsKMZ.aspx","allreports.kmz")

#doc_kml<-unz("allreports.kmz","doc.kml")

reports_xml<-read_xml("reports.kml")
report_nodes<-reports_xml %>% 
  xml_children() %>% 
  xml_children() %>% 
  .[3:5] %>%
  xml_children()

long<-report_nodes[4] %>% str_extract("(longitude>)([-0-9.]+)") %>% str_extract("[0-9.-]+") %>% as.numeric()
lat<-report_nodes[4] %>% str_extract("(latitude>)([-0-9.]+)") %>% str_extract("[0-9.-]+") %>% as.numeric()
date<-report_nodes[4] %>% str_extract("(when>)([-0-9.]+)") %>% str_replace("when>","") %>% as.Date()
#description<-report_nodes[4] %>% str_extract("(Report:)(.)(/b)") %>% str_replace("/b","")
