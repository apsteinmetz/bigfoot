...
states_map_52<-download.file("http://www2.census.gov/geo/tiger/GENZ2016/shp/cb_2016_us_state_20m.zip","gis/state_polygons.zip",mode="wb")
start_dir=getwd()
setwd("./gis")
...
state_sum$hover <- paste(str_to_title(state_sum$region),state_sum$value,"sightings")
...
p <- plot_geo(state_sum, locationmode = 'USA-states') %>%
  add_trace(
    z = ~value, text = ~hover, locations = ~region,
    color = ~value, colors = 'Greens'
  ) %>%
  colorbar(title = "Sighting Count") %>%
  layout(
    title = 'Bigfoot Sightings<br>(Hover for breakdown)',
    geo = g
  )
p
