...
states_map_50<-states_map_50_nested %>% 
  unnest()  %>% 
  select(id,region,long,lat)
```
```{r}
...
geo_data<-geo_data %>% mutate(region=as.character(NA))
```

Reverse geocode state names as "region" using point.in.poly function and summarise state sighting count by year.,
```{r}
...
state_sum<-bfro_data %>% group_by(region,Year) %>% summarise(count=n()) 
...
```
Bar chart of total US sightings, with a single year highlighted.
```{r}
#timeline of all sightings  
startYear<-1980
inputYr<-2001
fills<-c(rep("grey50",inputYr-startYear),
         "red",
         rep("grey50",thisYear-inputYr-1))
subsetYear<-state_sum %>% 
  filter(Year>=startYear,Year<=thisYear) %>% 
  group_by(Year) %>% 
  summarize(count=sum(count))
p<-ggplot(data=subsetYear)
p<-p+geom_col(aes(x=Year,y=count),fill=fills)
print(p)
